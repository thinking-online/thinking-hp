...
for k, msg in [("あなたのせいではありません", "読者を責めない一文がない"),
               ("1　4　9　1　6　2　5　3　6　4　9", "11個の数字の実演がない"),
               ("16　25　36　49", "数字の種明かしがない"),
               ("絶対にやらないこと", "タイトルの約束が宣言されていない"),
               ("才能", "才能の話に触れていない"),
               ("スポーツ", "勉強は外から見えないという話がない"),
               ("順番", "やめる→覚える→増やす、の順番の話がない"),
               ("バケツ", "順番を1枚で言うバケツの図がない")]:
    if k not in yomumae: bad("導入", f"読む前に に {msg}")
if len(re.findall(r"^## \d+ページ目$", yomumae, re.M)) != 9:
    bad("導入", "読む前に が9ページになっていない")
if len(yomumae) > 6000: bad("導入", "読む前に が長い。立ち読みで読み切れない")
# 量を否定していないか。本書は順番の話であって、努力を否定する本ではない
if "量を増やす" not in yomumae:
    bad("導入", "巻頭が「最後は量を増やす」を言っていない。量の否定になっている")
# 出題と種明かしが同じページにあると、読者が失敗を体験できない
_p = re.split(r"^## \d+ページ目$", yomumae, flags=re.M)
_q = next((i for i, x in enumerate(_p) if "1　4　9　1　6　2　5　3　6　4　9" in x), None)
_a = next((i for i, x in enumerate(_p) if "16　25　36　49" in x), None)
if _q is None or _a is None or _a <= _q:
    bad("導入", "種明かしが出題と同じページか、それより前にある。めくらせないと実演が死ぬ")
# 第1章は巻頭の実演を呼び戻すだけにする（二度やらない）
if "1　4　9　1　6　2　5　3　6　4　9" not in s1:
    bad("導入", "第1章に巻頭の11個の数字の呼び戻しがない")
if "16　25　36　49" in s1:
    bad("導入", "第1章で種明かしを繰り返している。巻頭で済んでいる")
