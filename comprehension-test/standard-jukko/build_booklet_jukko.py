#!/usr/bin/env python3
"""英文熟考70文の問題集PDF用HTMLを生成。各文: 問題ページ(英文のみ)→ 解答・解説ページ(訳あり)。"""
import json, html
from pathlib import Path

BASE = Path('/tmp/claude-0/-home-user-thinking-hp/5f12d2a1-7df4-5ffd-924b-ed23125eba30/scratchpad')
RES = BASE / 'results-jukko'
OUT = BASE / 'booklet-jukko.html'
N = 70

def esc(s): return html.escape(str(s))

entries = [json.load(open(RES / f'q{n}.json', encoding='utf-8')) for n in range(1, N + 1)]
...
.cover { text-align: center; padding-top: 65mm; }
.cover h1 { font-size: 25pt; letter-spacing: 1px; margin: 0 0 6px; }
.cover .sub { font-size: 12pt; color: #555; }
.cover .rule { font-size: 10pt; color: #888; margin-top: 38mm; line-height: 1.9; }
...
.yaku { font-size: 9.7pt; color: #444; padding: 5px 4px 0; margin-bottom: 14px; line-height: 1.75; }
...
.ansrow { margin: 6px 0 3px; }
.ansrow .lab { font-weight: bold; font-size: 10pt; }
.ansrow .val { font-size: 13pt; font-weight: bold; color: #b00000; margin-left: 6px; }
.correct-choice { background: #fdeaea; }
.exp { background: #f6f6f4; border: 1px solid #ccc; padding: 7px 11px; margin: 2px 0 12px; font-size: 9.6pt; line-height: 1.62; }
.exp .lbl { font-weight: bold; color: #333; }
.answer-block { break-inside: avoid; margin-bottom: 10px; }
.footer-note { text-align:center; font-size: 8.5pt; color: #aaa; margin-top: 6px; break-before: avoid; }
"""

def choices(q, correct=None):
...
parts.append("""
<div class="cover">
  <h1>英文解釈 確認問題集 〈標準編〉</h1>
  <div class="sub">英文熟考(上) 準拠 &nbsp;No.1 – 70</div>
  <div class="sub" style="font-size:10pt;margin-top:8px;">全問客観式(4択)</div>
  <div class="rule">
    ● 訳の暗記ではなく、英文の構造と内容を本当に理解できているかを問う問題です。<br>
    ● 各文は「問題ページ(英文のみ)」→「解答・解説ページ(和訳つき)」の順に独立して並んでいます。<br>
    ● まず問題を解き、次のページで答え合わせと解説・和訳を確認しましょう。
  </div>
</div>
""")
...
for e in entries:
    ...
    for i, q in enumerate(qs, 1):
        ...
        a.append(choices(q, correct=cor))
...
print('wrote', OUT, 'entries:', len(entries), 'questions:', sum(len(e['questions']) for e in entries))
