...
| `bars` | 配点の比重を横バーで見せる | `rows[{label,note,value,pct,style}]`, `max`, `total`, `unit`<br>`style`: `key`(金) / `mute`(灰) |
| `exam` | 例題（英文＋設問＋選択肢と判定根拠） | `passage`, `question`, `choices[{k,text,ok,why}]` |
...
### 模試の点数データが無い生徒（高1・高2など）

`score` ブロックは現在地の得点が前提なので、**まだ共通テスト形式で測れていない生徒には使わない。**
代わりに、次の順で「敵の数字」から設計を組み立てる（`students/uchiyama/` がこの型）。

1. **配点の全体像**（`table`）― 共テ・二次の科目と配点を確定させる
2. **合格ライン**（`kpis` + `table`）― 合格最低点と判定ラインを数字にする
3. **共テ×二次のシミュレーション**（`table`）― 共テの得点率ごとに、二次で何点必要かを計算する
4. **科目ごとの価値**（`bars`）― 総合点のなかでの各科目の重みを出す
5. **目標配点**（`table`）― 現在地ではなく、到達すべき得点を科目別に置く

現在地は「これから測るもの」として、最初の30日のチェックリストに入れる。
