...
├── _sample/                      # 営業・説明用サンプル（高3・国公立／営業ページ付き）
├── _sample-h2/                   # 営業・説明用サンプル（高2・国公立／模試データなし）
├── _sample-h3/                   # 営業・説明用サンプル（高3・国公立／模試データあり）
└── <生徒スラッグ>/
...
```bash
node students/_engine/build.mjs students/_sample      # 営業ページ付き
node students/_engine/build.mjs students/_sample-h2   # 高2・国公立
node students/_engine/build.mjs students/_sample-h3   # 高3・国公立
```

サンプルは2種類あります。相手や場面で使い分けてください。

| サンプル | 想定 | 構成 | 中身 |
|---|---|---|---|
| `_sample/` | **営業・初回説明** | 全14ページ（00・12が営業ページ） | 「何のための資料か」「面談から運用までの流れ」を前後に付けた版 |
| `_sample-h2/` | **高2・模試データがない生徒** | 全14ページ（全部が本体） | 配点と合格ラインから逆算する型。大学の公表配点を使った実例 |
| `_sample-h3/` | **高3・模試の得点がある生徒** | 全12ページ（全部が本体） | 模試の得点表→得点設計→科目別戦略の型。得点表の見え方を見せたい時はこれ |

`_sample-h2/` `_sample-h3/` は、実在の生徒向けに作ったシートを匿名化したものです。

- **`_sample-h2/`**：生徒名を架空（山田 花子）に置換。「入塾前テスト」など特定の実施回を指す表現も一般化。
  シートに実際の得点は入っていないため、配点・合格最低点・判定ラインは大学の公表データのまま使えます。
- **`_sample-h3/`**：生徒名を架空（鈴木 一郎）に置換したうえで、**模試の得点をすべて架空値に差し替え**ています。
  実在の生徒の成績をそのまま配布しないための措置です。合計・得点率・上積み・目標配点は
  差し替え後の得点から**すべて再計算**してあり、シート内の数値は完全に整合しています。

### 得点を差し替えるときの注意

得点は `score` ブロックだけでなく、**表紙のKPI・02の合計・04の得点設計表・科目別ページの得点率**にも
散らばっています。1箇所でも直し漏れると資料内で数字が食い違うので、差し替え後は必ず次を確認してください。

```bash
# 得点表から合計を再計算して、本文中の数値と突き合わせる
python3 - <<'EOF'
import json
d = json.load(open('students/<スラッグ>/data.json', encoding='utf8'))
rows = next(b['rows'] for p in d['pages'] for b in p['blocks'] if b.get('t')=='score')
cur, tgt = sum(r['conv'] for r in rows), sum(r['target'] for r in rows)
print(f"現在 {cur} ({cur/950*100:.1f}%) / 目標 {tgt} ({tgt/950*100:.1f}%) / 上積み +{tgt-cur:.1f}")
EOF
```
