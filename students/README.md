...
├── _sample-h2/                   # 営業・説明用サンプル（高2・国公立／シート本体のみ）
└── <生徒スラッグ>/
...
```bash
node students/_engine/build.mjs students/_sample      # 高3・国公立（営業ページ付き）
node students/_engine/build.mjs students/_sample-h2   # 高2・国公立（シート本体のみ）
```
...
| `_sample-h2/` | **シートそのものを見せたい時** | 全14ページ（全部が本体） | 高2向け。実際の大学の公表配点から設計した実例をそのまま見本化 |

`_sample-h2/` は実在の生徒向けに作ったシートを匿名化したものです。**生徒名は架空**（山田 花子）に置き換え、
「入塾前テスト」など特定の実施回を指す表現も一般化してあります。配点・合格最低点・判定ラインは
大学の公表データなので、そのまま実例として使えます。
