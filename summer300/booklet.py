...
SRC = os.path.join(ROOT, "summer300_words.csv")
...
.main{min-width:0}
.wline{display:flex;align-items:baseline;gap:7px;flex-wrap:wrap}
.w{font-family:Georgia,serif;font-size:14px;font-weight:700;color:var(--ink);letter-spacing:.01em}
...
cover = f'''<div class="page cover">
  <div>
    ...
    <h1>この夏に差をつける<br><span class="accent">英単語 300</span></h1>
    <div class="rule"></div>
    <div class="lead">LEAP・鉄壁・システム英単語・ターゲット・英検（準1級〜準2級）——
    主要単語帳を横断し、<b style="color:#fff">「どの単語帳にも共通して載っている＝入試・英検で必ず問われる」</b>
    最重要語だけを頻度で厳選しました。まずはこの300語から。</div>
  </div>
  <div>
    <div class="meta">
      <div class="box"><div class="num">300</div><div class="cap"> WORDS 厳選</div></div>
      <div class="box"><div class="num">7</div><div class="cap">単語帳を横断</div></div>
      <div class="box"><div class="num">★★★</div><div class="cap">重要度つき</div></div>
    </div>
  </div>
  <div>
    <div class="note">★の数＝掲載単語帳の多さ（重要度）。★★★＝7冊すべてに掲載。<br>
    □にチェックしながら、覚えた語を消していこう。</div>
...
for si, start in enumerate(range(0, total, SEC), 1):
  ...
  <div class="sec-head"><div class="l"><span class="sn">SECTION {si:02d}</span>
    <span class="tt">最重要英単語 300</span></div>
    <span class="rg">No.{int(lo):03d} – {int(hi):03d}</span></div>
  <div class="legend">重要度 <span class="st">★★★</span>=7冊掲載 / <span class="st">★★☆</span>=6冊 / <span class="st">★☆☆</span>=5冊　　□にチェックして暗記</div>
  <div class="grid">{items}</div>
  <div class="foot"><span class="b">THINKING</span><span>この夏に差をつける英単語300 &nbsp;—&nbsp; {si} / {(total+SEC-1)//SEC}</span></div>
</div>''')

doc = f'''<!doctype html><html lang="ja"><head><meta charset="utf-8">
<title>この夏に差をつける 英単語300</title><style>{CSS}</style></head>
